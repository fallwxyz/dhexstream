<?php 
    ...
    if($parts[2] == "ongoing"){
        $title = "On Going";
        $slug = "ongoing-anime" . $page;
    }
    else{
        $title = "Complete";
        $slug = "complete-anime" . $page;
    }
    ...
    $filtered = array_filter(
        $raw['data']['animeList'],
        function ($anime) use ($excludeTitles) {
            return !in_array($anime['animeId'], $excludeTitles);
        }
    );

    $raws = array_values($filtered);
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <main style="gap: 20px;">
        <div class="ongoing">
            ...
            <div class="home-list">
                ...
                foreach($raws as $row){
                    if($title == "On Going"){
                        $yow = $row['latestReleaseDate'];
                    }
                    else{
                        $yow = $row['lastReleaseDate'];
                    }
                ?>
                <a href="<?= $root ?>anime/<?= $row['animeId'] ?>" class="data">
                    <img src="<?= $row['poster'] ?>">
                    <div class="text">
                        <p id="date" style="display:flex;justify-content:space-between;"><span><?= $yow ?></span><span><?= $row['episodes'] ?> Eps</span></p>
            ...
            <div class="navy">
                ...
                if($raw['pagination']['hasPrevPage'] == false){
                    ...
                    $prevPage = "href='$root" . "watch/" . $parts[2] . "/" . $raw['pagination']['prevPage'] . "'";
                    $prevBtn = "";
                }
                ...
                if($raw['pagination']['hasNextPage'] == false){
                    ...
                    $nextPage = "href='$root" . "watch/" . $parts[2] . "/" . $raw['pagination']['nextPage'] . "'";
                    $nextBtn = "";
                }
                ?>
